...
def decode_data_format_3(data):
    ...
    pressure = unpack("!H", data[6:8])[0] + 50000

    acceleration_x = unpack("!h", data[8:10])[0]
    acceleration_y = unpack("!h", data[10:12])[0]
    acceleration_z = unpack("!h", data[12:14])[0]
    battery_voltage = unpack("!H", data[14:16])[0]
...
def decode_data_format_5(data):
    ...
    temperature = unpack("!h", data[3:5])[0] * 0.005

    humidity = unpack("!H", data[5:7])[0] * 0.0025

    pressure = unpack("!H", data[7:9])[0] + 50000

    acceleration_x = unpack("!h", data[9:11])[0]
    acceleration_y = unpack("!h", data[11:13])[0]
    acceleration_z = unpack("!h", data[13:15])[0]

    power_bin = bin(unpack("!H", data[15:17])[0])[2:]
    battery_voltage = int(power_bin[:11], 2) + 1600
    tx_power = int(power_bin[11:], 2) * 2 - 40

    movement_counter = data[18]

    measurement_sequence = unpack("!H", data[18:20])[0]
